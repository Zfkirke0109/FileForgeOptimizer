...
class OptimizeScreenTest {
    ...
    fun materialHostExposesOptimizeControlsAndAllDestinations() {
        ActivityScenario.launch(MainActivity::class.java).use {
            ...
            ).forEach { id -> onView(withId(id)).check(matches(isDisplayed())) }
            ...
            onView(withId(R.id.start_optimization)).check(matches(isDisplayed()))
        }
    }
    ...
    private fun assertRunningControlsVisible() {
        ...
        onView(withText("Movies/sample.mkv")).check(matches(isDisplayed()))
    }
}
